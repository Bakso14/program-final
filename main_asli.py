...
cv2.createTrackbar(co, 'thresh',5,360,nothing)
cv2.createTrackbar(th_sse, 'thresh',75,100,nothing)
cv2.createTrackbar(th_bin, 'thresh',183,255,nothing)
...
#while(1):
    ...
    R = 20 # maksimal dalam clstr
    N = 4 # lama cc dalam clstr
    ...
    for connected_component in range(v):
        for a in range(len(final_clstr)):
            if cluster_orientation[0,connected_component] < (final_clstr[a][0] + koreksi_o) and cluster_orientation[0,connected_component] > (final_clstr[a][0] - koreksi_o) and cluster_orientation[0,connected_component] != 0:
    ...
    c#v2.imshow('threshold',th1)
    #cv2.imshow('asli',src)
    
    
    #save gambar hasil
    #path_coba = r'D:\Mata_kuliah_s2\Thesis\Mulai\Program\paka_dataset\hasil_asli'
    #os.chdir(path_coba)
    #a_str = str(foto)
